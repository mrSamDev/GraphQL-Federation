...
import { dbHealthCheck, getAllMoviesRaw } from './db';
...
const bunServer = Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    // Internal REST endpoint for search sync
    if (url.pathname === '/movies/sync' && req.method === 'GET') {
      const rows = getAllMoviesRaw();
...
logger.info({ port: PORT }, 'movies service started');
